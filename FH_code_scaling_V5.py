...
#INITIAL CONDITIONS
Rb0    = 0.5 *kp  #initial blowout conditions
dRb0   = 0 #initial blowout radius
xi_0   = 0 # initial conditions for RHS
xi_max = 5.6 * kp #plasma wavelength = cavity length = 60 micro meters
dxi = 0.001 * kp # size of integration steps
steps = np.int(np.abs(xi_max/dxi)) # number of steps needed to achieve dxi 
initialvalue = [Rb0,dRb0]
#DRIVER PARAMETERS
E0 = max_E0(me,wp,qe)
sig_z = 0.155 * kp #7e-6 #standard deviation
sig_r = 0.1* kp #5e-6
mu = 0.5 * kp#where the peak is found on comoving axis
#r = 0.001 *kp#radius 
qd = 300e-12#charge of driver 300 pico C
mdd = max_driver_n(qd,qe,sig_r,sig_z) #/ pn #max charge density of driver

#DRIVER BEAM ELECTRON/CHARGE DENSITY
def DriverBeam(x=None,r=None, mu=None, sig_z=None, sig_r=None):
...
def modelWLu(Rb,xi,mu,sig_z,sig_r):
    rb,drb =Rb
    Lambda =DriverBeam(xi,rb,mu, sig_z,sig_r)  # A driver with gaussian distribution
...
solWLu = odeint(modelWLu, initialvalue,xi,args=(mu,sig_z,sig_r)); #(func,initial,t)
...
ax2b.plot(xi, DriverBeam(xi,r,mu,sig_z,sig_r),'--',color='blue')
